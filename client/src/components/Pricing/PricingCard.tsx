...
type PricingCardProps = {
    ...
    features: string[];        // Features is an array of strings
    notAllowed?: string[];      // notAllowed is optional and a boolean
  };
  
const PricingCard: React.FC<PricingCardProps>= ({price,caption,features, notAllowed})=>{
    return(
        <>
        <div className="w-full max-w-sm p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-8 dark:bg-gray-800 dark:border-gray-700">
                    <h5 className="mb-4 text-xl font-medium text-gray-500 dark:text-gray-400">{caption}</h5>
                    ...
                    <ul role="list" className="space-y-5 my-7">
                       ...
                       {
                        notAllowed && notAllowed.map((item)=>(
